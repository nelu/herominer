...
class HeroManager:
    ...
    def update_heroes_stats(self):
        ...
        close_game()
        menus.open_menu("heroes")
        for hero in heroes:
            log.debug(f"update_heroes_stats: updating {hero._slug}")
            hero.set_search(False)
            hero.artifacts.set_open_settings(update_stats=True)
            update_result = driver.start('heroes/update-stats')
            ...
            # retry failure
            if not update_result and not (back_to_lobby() and menus.open_menu("heroes")):
                log.error(
